...
class TestStockLedgerReport(FrappeTestCase):
    ...
    def test_stock_ledger_reflects_receipt(self):
        # create a stock entry (issue)
        doc = frappe.get_doc(
            {
                "doctype": "Stock Entry",
                "type": "Receipt",
                "posting_date": "2025-05-17",
                "to_warehouse": self.warehouse.name,  # <-- Correct field
                "items": [
                    {"item": self.item.name, "quantity": 2, "valuation_rate": 15000}
                ],
            }
        ).insert()
        doc.submit()
        self.issue_name = doc.name
        ...
        self.assertTrue(
            any(
                row["voucher_no"] == self.issue_name and row["item"] == self.item.name
                for row in data
            ),
            "Stock Ledger Report should include the issue entry.",
        )
